...
for i in range(len(gvgal)):
    for j in range(len(regions)):
        ...
        if np.abs(gvgal.l.degree[i] - float(regions[j])) < 0.9:
            ...
            plt.imshow(img, vmin=0, vmax=np.max(img), origin='lower', interpolation='nearest', aspect='auto', cmap=cm.gray)
            plt.savefig('../images/varstamps/var_'+str(gv['uploadID'][i])+'.png')
            plt.close()
