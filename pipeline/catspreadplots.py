...
for curregion in skyrange:
    ...
    # Load maps
    #################################################
    img = fits.open('../Dunmaps/countmaps/count_map_name_'+curregion.replace('.', '')+'_gal_sec_in.fits')[0].data
    ...
    ax2.scatter(t1gais['nuv_galex'], t1gais['nuv_sex']-t1gais['nuv_galex'], alpha=0.5, edgecolor='none', facecolor='black', s=1)
    ...
    ax3.set_xlim((12, 25))
    ax3.set_ylim(0, 4)
    ax3.set_xlabel('NUV (only SExtractor)')
    ...
    ax4.set_xlim((12, 25))
    ax4.set_ylim((3,  12))
    ax4.set_xlabel('NUV (Only SExtractor),  N = '+str(len(t1)))
    ax4.set_ylabel('FWHM')

    # exp, background and counts vs gl
    ax5.plot(gbrange, imgvals*1000., label='counts*1000', c='green', linestyle='--')
    ax5.plot(gbrange, expmapvals, label='expmap', c='blue')
    ax5.plot(gbrange, bkgdvals*1000. - 10., label='bkgd*1000', c='red', linewidth=2)
    ax5.plot(bins_mean, n/100. - 20, label='# sources/100', c='black', linewidth=2)
    ax5.set_xlim((-10, 10))
    if np.max(expmapvals) > 120:
        ax5.set_ylim(-22, np.max(expmapvals) + 2)
    else:
        ax5.set_ylim(-22, 120)
    ax5.set_xlabel('gb')
    ax5.set_ylabel('counts')
    ax5.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=4, mode="expand", borderaxespad=0., fontsize=15)
    plt.suptitle('region = '+curregion)
    plt.tight_layout()
    plt.savefig('../images/06-22-matchplots/region'+curregion+'matchplots.png')
    #plt.show()
    plt.close()
