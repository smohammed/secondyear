...
#cut = np.where((gv['fov_radius'] < 0.59) & (gv['distance_arcmin'] == 0))
#gv = gv[cut]

gv = Table.read('../raddiwd.txt', format='ascii')

#gvgal = SkyCoord(gv['ra']*u.deg, gv['dec']*u.deg, frame='icrs').galactic
gvgal = SkyCoord(gv['gl']*u.deg, gv['gb']*u.deg, frame='galactic')

for i in range(len(gvgal)):
    print 'i = ', i
    for j in range(len(regions)):
        region = regions[j]
        if np.abs(gvgal.l.degree[i] - float(regions[j])) < 0.5:
            l, b = gvgal.l.degree[i], gvgal.b.degree[i]
            #if not os.path.isfile('../images/varstamps/var_'+str(gv['uploadID'][i])+'_gl_'+str(l)[:5]+'.png'):
            if not os.path.isfile('../images/raddiwd/wd_gl_'+str(gv['gl'][i])+str(gv['gb'][i])+'.png'):
                if region == '0.5':
                    region = '5'
                ...
                #plt.title('NUV_sex = '+str(gv['nuv_sex'][i])[:5]+', NUV_GAIS = '+str(gv['nuv_galex'][i])[:5])
                plt.title('Raddi WD gl_'+str(gv['gl'][i])[:6]+'_gb_'+str(gv['gb'][i])[:6])
                #plt.show()
                #plt.savefig('../images/varstamps/var_'+str(gv['uploadID'][i])+'_gl_'+str(l)[:5]+'.png')
                plt.savefig('../images/raddiwd/wd_gl_'+str(gv['gl'][i])[:6]+'_gb_'+str(gv['gb'][i])[:6]+'.png')
                plt.close()
                print 'j = ', region
                print 'xmin, xmax = ', xmin, xmax
                print 'ymin, ymay = ', ymin, ymax


#i =  13
#j =  28.4
#xmin, xmax =  748.807374999 948.807374999
#ymin, ymay =  48233.0822763 48433.0822763
