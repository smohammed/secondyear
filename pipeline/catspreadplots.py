...
for curregion in skyrange:
    ...
    t1 = Table.read('../Dunmaps/fwhm/starcat_'+curregion+'mapweight_fec_fwhm.txt', format='ascii')
    ...
    # Load maps
    #################################################
    #img = fits.open('../Dunmaps/countmaps/count_map_name_'+curregion.replace('.', '')+'_fec_gal_sec_in.fits')[0].data

    # New fec maps
    img = fits.open('../count_map_'+curregion.replace('.', '')+'-cal-sec_in.fits')[0].data
    img = img[im1ymin:im1ymax, im1xmin:im1xmax]

    #expmap = fits.open('../../galexfiles/exp/count_map_name_'+curregion.replace('.', '')+'_fec_gal_sec_exp.fits')[0].data

    # New fec exp map
    expmap = fits.open('../count_map_'+curregion.replace('.', '')+'-cal-sec_exp.fits')[0].data
    expmap = expmap[im1ymin:im1ymax, im1xmin:im1xmax]
    bkgd = fits.open('../Dunmaps/background/background_im1_'+curregion.replace('.', '')+'_fec.fits')[0].data

    #hdulist = fits.open('../Dunmaps/countmaps/count_map_name_'+curregion.replace('.', '')+'_fec_gal_sec_in.fits')
    hdulist = fits.open('../count_map_'+curregion.replace('.', '')+'-cal-sec_in.fits')
    ...
    plt.tight_layout()
    plt.savefig('../region'+curregion+'matchplots_fec.png')
    #plt.show()
    plt.close()
