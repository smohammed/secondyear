...
def phot(glinit, glend, gbinit, gbend):
    intmap = np.sqrt(fits.open('../intmapcsvcorr12000_gl_'+str(glinit)+'to'+str(glend)+'_gb_'+str(gbinit)+'to'+str(gbend)+'.fits')[0].data)
    tycho = fits.open('../tychobstarmatch.fits')[1].data
    tcut = np.where((tycho.gl > glinit) & (tycho.gl < glend) & (tycho.gb > gbinit) & (tycho.gb < gbend) & (tycho.nuv_cps > 10.) & (tycho.nuv_cps < 100.))
    tycho = tycho[tcut]
    tychogl = (tycho.gl - glinit) * 12000. / 10.
    tychogb = (tycho.gb - gbinit) * 12000. / 10.
