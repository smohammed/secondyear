...
lines = loadtxt("fits_photon_list.txt", comments = "#", delimiter = ",", unpack = False, dtype = str)
...
photons = fits.open(path+lines[0])[1].data
photons2 = fits.open(path+lines[1])[1].data
...
for i in range(startgl+2,endgl,2):
	...
	glinit = np.concatenate((f1.gl_cor,f2.gl_cor))
	gbinit = np.concatenate((f1.gb_cor,f2.gb_cor))
