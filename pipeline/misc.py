...
i = 120
gbrange = 5.
scut = np.where((np.abs(star.gb) > gbrange))
scut2 = np.where((np.abs(star.gb) < gbrange))
ncut = np.where(np.abs(newt['gb']) > gbrange)
ncut2 = np.where(np.abs(newt['gb']) < gbrange)
f, (ax1, ax2) = plt.subplots(1, 2)
plt.rc('legend',**{'fontsize':15})
a1 = ax1.scatter(star.j[scut]-star.k[scut],star.nuv[scut]-star.j[scut],edgecolor='none',alpha=0.5)
...
a2 = ax1.scatter(newt['j_m'][ncut]-newt['k_m'][ncut],newt['nuv_mag'][ncut]-newt['j_m'][ncut],facecolor='none',edgecolor='red',s=30,alpha=0.1)
...
ax1.legend([a1,a2,a3],['GALEX sextractor','galex','pickles'],scatterpoints=1,loc=2)


b1 = ax2.scatter(star.j[scut2]-star.k[scut2],star.nuv[scut2]-star.j[scut2],edgecolor='none',alpha=0.5)
...
b2 = ax2.scatter(newt['j_m'][ncut2]-newt['k_m'][ncut2],newt['nuv_mag'][ncut2]-newt['j_m'][ncut2],facecolor='none',edgecolor='red',s=30,alpha=0.1)
...
ax2.legend([b1,b2,b3],['GALEX sextractor','galex','pickles'],scatterpoints=1,loc=2)
plt.show()
