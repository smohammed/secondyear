...
for curregion in skyrange:
    ...
    if fec == 1:
        t1 = Table.read('../Dunmaps/fwhm/fec/starcat_'+curregion+'mapweight_fec_fwhm.txt', format='ascii')
    ...
    if fec == 0:
        ...
        img = hdulist[0].data
        expmap = fits.open('../../galexfiles/exp/count_map_name_'+curregion.replace('.', '')+'gal_sec_exp.fits')[0].data
        bkgd = fits.open('../Dunmaps/background/background_im1_'+curregion.replace('.', '')+'.fits')[0].data

    # New fec maps
    if fec == 1:
        hdulist = fits.open('../fecmaps/count_map_'+curregion.replace('.', '')+'-cal-sec_in.fits')
        img = hdulist[0].data
        expmap = fits.open('../fecmaps/count_map_'+curregion.replace('.', '')+'-cal-sec_exp.fits')[0].data
    ...
    if fec == 0:
        plt.savefig('../11-14-region'+curregion+'matchplots.png')

    if fec == 1:
        plt.savefig('../11-14-region'+curregion+'matchplots_fec.png')
