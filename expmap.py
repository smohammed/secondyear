...
files1 = np.loadtxt('scst_list.txt',dtype='string')
#files2 = np.loadtxt('csv_list_2.txt')
...
for line in range(len(files1)):
	d = fits.open('scst/'+files1[line])[1].data
	...
	#gx = gx[np.where((gx > 0.) & (gx < 360.*60.))]
	#gy = gy[np.where((gy > 0.) & (gy < 20.*60.))]
